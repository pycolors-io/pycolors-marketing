...
Use the `error` prop for validation errors. It automatically sets `aria-invalid`, `aria-errormessage`, and error styling. The error text carries `role="alert"`, so it is announced immediately when it appears — including while the field is already focused. A consumer-supplied `aria-describedby` is composed with the field's own helper/error ids rather than replaced.
...
- `error` text has `role="alert"` and is announced live, even while the field is already focused.
