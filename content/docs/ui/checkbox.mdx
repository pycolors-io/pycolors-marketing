...
<Callout title="Core idea">
  Checkbox is for explicit selection. Use it when each option can be
...
export function BasicCheckbox() {
  return (
    <CheckboxField>
      <Checkbox id="analytics" />
      <CheckboxContent>
        <CheckboxLabel htmlFor="analytics">Enable analytics</CheckboxLabel>
        <CheckboxDescription>
          Collect product usage data to improve the dashboard experience.
        </CheckboxDescription>
      </CheckboxContent>
...
<Callout type="info" title="Label rule">
  Always connect the visible label with the checkbox using `htmlFor` and `id`,
  unless the label wraps the control directly.
</Callout>
...
<DocsFeatureGrid
  ...
  items={[
    ...
    {
      title: "Consent",
      description: "Use for terms, confirmations, and explicit opt-in choices.",
      icon: ShieldCheck,
    },
  ]}
/>
...
<Callout title="State rule">
  Radix Checkbox can return `true`, `false`, or `"indeterminate"`. Convert the
  value intentionally when your app expects a boolean.
</Callout>
...
<Callout type="info" title="Disabled copy">
  When a disabled option may frustrate users, explain why it is unavailable and
  what unlocks it.
</Callout>

### Form validation

Use the `error` prop when a required checkbox is not selected. It renders visible
error copy, sets `aria-invalid`, and connects the message with both
`aria-describedby` and `aria-errormessage`. The error has `role="alert"`, so it
is announced when it appears. Consumer-provided `aria-describedby` values are
preserved alongside the error message id.
...
<Callout title="Decision rule">
  Use Checkbox for selection, Switch for immediate enablement, and Radio Group
  for one exclusive choice.
</Callout>
...
| `error`           | `string`                     | —       | Renders an announced error and sets the invalid state |
...
- `error` renders visible error copy and wires `aria-invalid`,
...
- use the `error` prop for invalid required confirmations
- keep checkbox labels short and action-oriented

<Callout title="Accessibility rule">
  A checkbox needs a clear accessible name. The label should make sense even
  when read without surrounding context.
</Callout>
...
<Accordion title="When should I use indeterminate?">
  Use indeterminate when a parent checkbox represents a group where only some
  child items are selected.
</Accordion>

<Accordion title="Can Checkbox be used inside forms?">
  Yes. Use `name`, `value`, `required`, and validation copy when the checkbox
