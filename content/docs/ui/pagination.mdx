...
`PaginationLink`, `PaginationPrevious`, and `PaginationNext` support the
`asChild` composition pattern. Pass a real anchor (or a framework `Link`) as
the child and the control renders it as the root element while preserving all
styling, `data-slot`, accessible labels, and — for `PaginationPrevious` and
`PaginationNext` — the built-in icon and visible text.
...
```tsx title="pagination-link-composition.tsx"
import {
  Pagination,
  PaginationContent,
  PaginationItem,
...
// With real <a> elements — swap for your framework Link:
<Pagination>
  <PaginationContent>
    <PaginationItem>
      <PaginationPrevious asChild>
...
<Callout title="Framework links">
  Replace `<a href="...">` with your router's `Link` component (Next.js, React
  Router, TanStack Router, etc.) — the composition pattern is router-neutral.
</Callout>

<Callout type="info" title="PaginationPrevious and PaginationNext with asChild">
  You only need to supply the anchor element as the child. The built-in
  ChevronLeft / ChevronRight icon and the visible Previous / Next text are
  rendered automatically alongside your child element.
</Callout>
...
<Callout type="info" title="Architecture rule">
  Pagination should receive state and callbacks. The feature layer should own
  the data source, filters, sorting, and URL sync.
</Callout>
...
<Callout title="Decision rule">
  Use Pagination when the user benefits from a stable position in a finite
  result set.
</Callout>
...
| ----------- | --------- | ----------- | ----------------------------------------------------------------------------- |
| `asChild`   | `boolean` | `false`     | Render as a Slot wrapper; supply a real anchor or framework link as the child |
...
| Prop        | Type      | Default           | Description                                                    |
| ----------- | --------- | ----------------- | -------------------------------------------------------------- |
| `asChild`   | `boolean` | `false`           | Render as a Slot wrapper; built-in icon and text are preserved |
| `label`     | `string`  | `"Previous page"` | Accessible label for screen readers                            |
| `className` | `string`  | —                 | Additional Tailwind classes                                    |
...
| ----------- | --------- | ------------- | -------------------------------------------------------------- |
| `asChild`   | `boolean` | `false`       | Render as a Slot wrapper; built-in icon and text are preserved |
| `label`     | `string`  | `"Next page"` | Accessible label for screen readers                            |
| `className` | `string`  | —             | Additional Tailwind classes                                    |
...
<Callout title="Accessibility rule">
  Pagination controls should be navigable, understandable, and predictable
  without relying on visual position alone.
</Callout>
...
<Accordion title="Can Pagination work with cursor-based APIs?">
  Yes, but the cursor logic belongs to the data layer. Use Previous and Next
  controls if numeric pages do not exist.
</Accordion>

<Accordion title="When should I hide Pagination?">
  Hide it when there is only one page, when the data set is tiny, or when an
  ...
  items={[
    {
      title: "Table",
      href: "/docs/ui/table",
      description: "Render structured data that often pairs with Pagination.",
      icon: Table2,
    },
    {
    ...
    },
    {
      ...
      description: "Place pagination inside real dashboard layouts.",
      icon: LayoutTemplate,
    },
    {
      title: "Empty, loading, and error states",
      href: "/docs/guides/empty-loading-error",
      description: "Handle async states around paged data.",
      icon: LoaderCircle,
    },
  ]}
/>
