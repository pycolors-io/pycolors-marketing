...
The resulting `role` and `aria-live` are deterministic: they are always derived from `ariaLive` and take precedence over any native `role` or `aria-live` value passed through the rest of the component's props.
...
- The computed `role` and `aria-live` cannot be silently overridden by other props; `ariaLive` is the single source of truth.
