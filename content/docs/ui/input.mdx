...
- `aria-invalid` is set
- error text is connected to the field via `aria-describedby` and `aria-errormessage`
- the error text has `role="alert"`, so it is announced immediately when it appears — including while the field is already focused, not only on the next focus event
- a consumer-supplied `aria-describedby` is composed with the field's own helper/error ids rather than replaced, so both descriptions remain available to assistive technology
...
- error text is also announced live via `role="alert"` the moment it appears
- `disabled` dims the field and shows a not-allowed cursor on the field itself, not only on the native attribute — the visual state and the functional state always match
