...
| `info`        | Informational status — semantic neutral, uses card tokens |

<Callout type="info" title="Design rule">
  Prefer `muted` and `outline` for metadata. Reserve semantic state variants for
...
<Callout title="Density rule">
  Use `sm` in tables and dense product views. Use `md` for most UI surfaces. Use
  `lg` sparingly.
</Callout>
...
<Callout title="Placement rule">
  Use left icons for state or identity. Use right icons for direction,
  continuation, or linked context.
</Callout>
...
<Callout title="Interaction rule">
  Only use interactive badges when the underlying element is semantic, such as
  an anchor or button. If it triggers an important action, use Button instead.
</Callout>
...
<Callout title="Decision rule">
  Badge communicates context. Button triggers intent. Alert explains important
  feedback.
</Callout>
...
- `info` uses existing semantic card tokens; contrast is theme-dependent and must satisfy the same accessibility requirements as other Badge variants.
...
<Accordion title="Should every status use a colored badge?">
  No. Use semantic color only when the state needs emphasis. For low-priority
  metadata, prefer `muted` or `outline`.
</Accordion>

<Accordion title="Can badges include icons?">
  Yes. Icons can reinforce meaning, but the text must still communicate the
  state.
</Accordion>

<Accordion title="Should icons appear on the left or the right?">
  Use left icons for state, identity, or category. Use right icons for
  direction, continuation, or linked context.
</Accordion>
