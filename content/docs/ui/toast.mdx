---
title: Toast
description: Non-blocking feedback messages used to confirm actions or display short system messages.
lastUpdated: "2026-04-28"
appliesTo: UI
---

import { Callout } from "fumadocs-ui/components/callout";
import { Accordion, Accordions } from "fumadocs-ui/components/accordion";
import { Step, Steps } from "fumadocs-ui/components/steps";
import { Tabs, Tab } from "fumadocs-ui/components/tabs";
import { Badge, Card, CardContent, CardHeader, CardTitle } from "@pycolors/ui";
import {
  Accessibility,
  ArrowRight,
  Bell,
  CheckCircle,
  CircleAlert,
  Info,
  MessageSquare,
  PanelTop,
  RefreshCcw,
  ShieldAlert,
  XCircle,
} from "lucide-react";
import { PreferAvoid } from "@/components/docs/prefer-avoid";
import { DocsLinks } from "@/components/docs/docs-links";
import { DocsFeatureGrid } from "@/components/docs/docs-feature-grid";
import { DocsConceptTabs } from "@/components/docs/docs-concept-tabs";

import { ClientOnly } from "../previews/client-only";
import {
  ToastTriggerPreview,
  ToastVariantsPreview,
} from "../previews/toast-previews";

<div className="flex flex-wrap items-center gap-2">
  <Badge variant="outline">UI</Badge>
  <Badge>Toast</Badge>
</div>

## Non-blocking feedback for product actions

The **Toast** component displays short, temporary, non-blocking feedback.

Use it to:

- confirm actions
- summarize background results
- show short system messages
- provide lightweight retry or undo feedback
- acknowledge user intent without interrupting the flow

Toast is intentionally minimal in PyColors UI.

It provides structure and accessibility, not a full notification system with queues, global stores, deduping, or persistent notification history.

<Callout title="Core idea">
  Toasts are temporary feedback. They should confirm, summarize, or recover —
  not block the task.
</Callout>

### Import

```tsx title="toast-import.tsx"
import { Toast } from "@pycolors/ui";
```

### Basic usage

Toast is controlled explicitly by the page or component.

Wrap your app, route, or layout subtree with `ToastProvider`, then render one `ToastViewport` per provider scope.

<Tabs items={["Preview", "Code"]}>
  <Tab value="Preview">
   <Preview>
    <ClientOnly fallback={<div className="text-sm text-muted-foreground">Loading preview…</div>}>
      <ToastTriggerPreview />
    </ClientOnly>
    </Preview>
  </Tab>

  <Tab value="Code">

```tsx title="toast-example.tsx"
"use client";

import * as React from "react";
import {
  Button,
  Toast,
  ToastProvider,
  ToastViewport,
  ToastTitle,
  ToastDescription,
} from "@pycolors/ui";

export function ToastExample() {
  const [open, setOpen] = React.useState(false);

  return (
    <ToastProvider>
      <Button onClick={() => setOpen(true)} variant="outline" size="sm">
        Trigger toast
      </Button>

      <Toast
        open={open}
        onOpenChange={setOpen}
        variant="success"
        duration={2500}
      >
        <ToastTitle>Changes saved</ToastTitle>
        <ToastDescription>Your settings were updated.</ToastDescription>
      </Toast>

      <ToastViewport className="fixed bottom-4 right-4 z-50 flex max-w-sm flex-col gap-2 outline-none" />
    </ToastProvider>
  );
}
```

  </Tab>
</Tabs>

<Callout type="info" title="Provider rule">
  Render one `ToastViewport` per provider scope. Avoid rendering a viewport
  inside every feature component.
</Callout>

## When to use Toast

Use Toast when feedback should be noticeable but should not interrupt the user.

<DocsFeatureGrid
  columns={3}
  tone="platform"
  items={[
    {
      title: "Confirmation",
      description:
        "Confirm that a save, update, archive, or background action completed.",
      icon: CheckCircle,
      tone: "success",
    },
    {
      title: "Retry or undo",
      description:
        "Offer lightweight recovery for reversible actions or failed background updates.",
      icon: RefreshCcw,
    },
    {
      title: "System feedback",
      description:
        "Show short system feedback that does not belong permanently in the page.",
      icon: Bell,
    },
  ]}
/>

### Variants

Toast supports a small set of variants. Success, warning and destructive variants
use semantic border colors with the normal foreground text, preserving readable
message contrast in both themes. Include a descriptive title or message so meaning
does not depend on color alone. Consumer `className` overrides still take precedence.

Keep variants limited to avoid turning every message into an event.

<Tabs items={["Preview", "Code"]}>
  <Tab value="Preview">
    <Preview>
    <ClientOnly fallback={<div className="text-sm text-muted-foreground">Loading preview…</div>}>
      <ToastVariantsPreview />
    </ClientOnly>
    </Preview>
  </Tab>

  <Tab value="Code">

```tsx title="toast-variants.tsx"
<Toast variant="default">
  <ToastTitle>Default</ToastTitle>
</Toast>

<Toast variant="success">
  <ToastTitle>Success</ToastTitle>
</Toast>

<Toast variant="warning">
  <ToastTitle>Warning</ToastTitle>
</Toast>

<Toast variant="destructive">
  <ToastTitle>Error</ToastTitle>
  <ToastDescription>
    Something went wrong.
  </ToastDescription>
</Toast>

<Toast variant="info">
  <ToastTitle>Information</ToastTitle>
</Toast>
```

  </Tab>
</Tabs>

### Available variants

| Variant       | Purpose                                                    |
| ------------- | ---------------------------------------------------------- |
| `default`     | Neutral confirmation or information                        |
| `success`     | Positive confirmation                                      |
| `warning`     | Attention needed, still non-blocking                       |
| `destructive` | Non-blocking error feedback                                |
| `info`        | Informational message — announces politely (non-assertive) |

<Callout type="info" title="Variant rule">
  Use `destructive` only for recoverable or non-blocking failures. Use Alert for
  blocking errors.
</Callout>

### With description

Use `ToastDescription` when the title alone is not enough.

```tsx title="toast-description.tsx"
<Toast variant="warning">
  <ToastTitle>Unsaved changes</ToastTitle>
  <ToastDescription>
    Save before leaving this page to avoid losing your work.
  </ToastDescription>
</Toast>
```

Keep the description short.

If the message needs more explanation, it probably belongs in an Alert, Dialog, or page section.

### Usage patterns

<Steps>
  <Step>
    #### Trigger after the action starts or completes

    Use toast feedback after a meaningful action: saved, archived, restored, copied, failed, or queued.

  </Step>

  <Step>
    #### Keep the message short

    Toasts are temporary. The user should understand them in one glance.

  </Step>

  <Step>
    #### Use inline state for active work

    For loading or submission, prefer button text such as `Saving…` before showing the final toast.

  </Step>

  <Step>
    #### Use recovery actions carefully

    Undo and retry work well for reversible actions, but they require real backend support.

  </Step>
</Steps>

## Toast vs Alert vs Dialog

| Situation                          | Use    |
| ---------------------------------- | ------ |
| Save confirmation                  | Toast  |
| Copied to clipboard                | Toast  |
| Background sync completed          | Toast  |
| Blocking page error                | Alert  |
| Form-level validation issue        | Alert  |
| Destructive confirmation           | Dialog |
| User must choose before continuing | Dialog |

<Callout title="Decision rule">
  Toast is for feedback after a flow. Alert is for information inside a flow.
  Dialog is for blocking decisions.
</Callout>

### Product guidance

<DocsConceptTabs
  items={[
    {
      value: 'Save',
      title: 'Save confirmation',
      icon: CheckCircle,
      description: (
        <>
          <p>
            Use toast feedback after the save completes.
            Keep loading state inline on the submit button.
          </p>

          <div className="mt-3 rounded-[5px] border border-border-subtle bg-muted/30 px-3 py-2 text-sm text-muted-foreground">
            Changes saved successfully.
          </div>
        </>
      ),
    },
    {
      value: 'Delete',
      title: 'Delete flow',
      icon: ShieldAlert,
      description: (
        <>
          <p>
            Confirm destructive intent in a Dialog first.
            Use toast feedback afterward for confirmation or Undo.
          </p>

          <div className="mt-3 rounded-[5px] border border-destructive/20 bg-destructive/5 px-3 py-2 text-sm text-destructive">
            Project deleted.
          </div>
        </>
      ),
    },
    {
      value: 'Retry',
      title: 'Retry flow',
      icon: RefreshCcw,
      description: (
        <>
          <p>
            Use retry for recoverable background failures.
            Avoid hiding critical failures only inside toast notifications.
          </p>

          <div className="mt-3 rounded-[5px] border border-border-subtle bg-muted/30 px-3 py-2 text-sm text-muted-foreground">
            Failed to sync data. Try again.
          </div>
        </>
      ),
    },
    {
      value: 'Undo',
      title: 'Undo flow',
      icon: MessageSquare,
      description: (
        <>
          <p>
            Use Undo for reversible actions such as archive,
            remove, delete, or bulk actions.
          </p>

          <div className="mt-3 rounded-[5px] border border-border-subtle bg-muted/30 px-3 py-2 text-sm text-muted-foreground">
            Member removed. Undo
          </div>
        </>
      ),
    },

]}
/>

## API

| Prop           | Type                                                   | Default       | Description                      |
| -------------- | ------------------------------------------------------ | ------------- | -------------------------------- |
| `variant`      | `"default" \| "success" \| "warning" \| "destructive"` | `"default"`   | Visual style                     |
| `open`         | `boolean`                                              | —             | Controlled open state            |
| `onOpenChange` | `(open: boolean) => void`                              | —             | State handler                    |
| `duration`     | `number`                                               | Radix default | Auto-close delay in milliseconds |
| `className`    | `string`                                               | —             | Custom Tailwind classes          |

Toast extends Radix Toast root props:

```ts title="toast-types.ts"
React.ComponentPropsWithoutRef<typeof ToastPrimitive.Root>;
```

### ToastProvider

Wraps the area where toasts are used.

```tsx title="toast-provider.tsx"
<ToastProvider>{/* app or route subtree */}</ToastProvider>
```

### ToastViewport

Defines where toasts appear on screen.

Render one viewport per provider scope.

```tsx title="toast-viewport.tsx"
<ToastViewport className="fixed bottom-4 right-4 z-50 flex max-w-sm flex-col gap-2 outline-none" />
```

### TypeScript types

```ts title="toast-types.ts"
export type ToastVariant = "default" | "success" | "warning" | "destructive";

export interface ToastProps
  extends
    React.ComponentPropsWithoutRef<typeof ToastPrimitive.Root>,
    VariantProps<typeof toastVariants> {}
```

### Accessibility

- Toast is built on Radix Toast primitives.
- Toasts should not trap focus.
- Toasts should not interrupt form input.
- Toasts are announced as live-region updates, at an urgency that matches `variant`:

  | Variant       | Announcement                       | Radix `type`   |
  | ------------- | ---------------------------------- | -------------- |
  | `default`     | Polite — waits for a pause         | `"background"` |
  | `success`     | Polite — waits for a pause         | `"background"` |
  | `warning`     | Assertive — interrupts immediately | `"foreground"` |
  | `destructive` | Assertive — interrupts immediately | `"foreground"` |
  | `info`        | Polite — waits for a pause         | `"background"` |

  Pass an explicit `type` prop to override this default on a specific toast.

- Keep copy short so announcements are understandable.
- Do not rely on color alone for destructive or warning messages.
- Do not use Toast as the only place for critical errors.

<Callout title="Accessibility rule">
  Toasts are helpful when they are brief. Long or critical messages should move
  into the page with Alert.
</Callout>

## Prefer / avoid

<PreferAvoid
  prefer={[
    "short confirmation messages",
    "one toast per meaningful action",
    "inline loading before toast completion",
    "retry or undo only when supported",
    "Alert for blocking failures",
  ]}
  avoid={[
    "using Toast for field validation",
    "hiding blocking errors in Toast only",
    "stacking many toasts at once",
    "long paragraphs inside toast descriptions",
    "using Toast as a notification center",
  ]}
/>

### When not to use Toast

Do not use Toast for:

- blocking errors
- field-level validation
- onboarding flows
- persistent notification centers
- payment failures that block progress
- permission errors that require explanation
- important messages the user must read before continuing

Use Alert, EmptyState, Dialog, or a dedicated pattern instead.

## Common questions

<Accordions type="single">
  <Accordion title="Should Toast handle global notification state?">
    Not by default. PyColors Toast is intentionally minimal. Add a global store only when your product needs queueing, deduping, persistence, or notification history.
  </Accordion>

<Accordion title="Can Toast include actions?">
  Yes, but keep actions short and recoverable, such as Undo or Retry. If the
  user must make a decision, use Dialog.
</Accordion>

<Accordion title="Should errors use Toast?">
  Only non-blocking or recoverable errors. Blocking errors should use Alert in
  context.
</Accordion>

  <Accordion title="How many ToastViewports should I render?">
    Usually one per app or layout scope. Avoid rendering a viewport in every component.
  </Accordion>
</Accordions>

## Related

<DocsLinks
  variant="list"
  tone="platform"
  items={[
    {
      title: "Alert",
      href: "/docs/ui/alert",
      description:
        "Inline feedback for blocking messages and persistent product states.",
      icon: CircleAlert,
    },
    {
      title: "Dialog",
      href: "/docs/ui/dialog",
      description:
        "Use Dialog when the user must confirm or complete a focused task.",
      icon: PanelTop,
    },
    {
      title: "Async actions pattern",
      href: "/docs/patterns/async-actions",
      description:
        "Design save, retry, optimistic UI, undo, and mutation feedback.",
      icon: RefreshCcw,
    },
    {
      title: "Accessibility",
      href: "/docs/ui/accessibility",
      description:
        "Build accessible feedback with clear semantics and announcements.",
      icon: Accessibility,
    },
  ]}
/>
