...
Configure the [public UI package and tokens](/docs/ui/installation), then create `src/components/blocks/sign-up/index.tsx` in your application. Open the source below and copy the complete file. Use the code block's copy button or select the code manually when clipboard access is unavailable.
...
Explore the [Blocks catalog](/blocks), use [Sign in](/docs/blocks/auth/sign-in) for returning users, or [compare Starters](/starters) for a complete application foundation.
