...
Configure the [public UI package and tokens](/docs/ui/installation), then create `src/components/blocks/sign-in/index.tsx` in your application. Open the source below and copy the complete file. Use the code block's copy button or select the code manually when clipboard access is unavailable.
...
Explore the [Blocks catalog](/blocks), compare the related [Sign up](/docs/blocks/auth/sign-up) and [Password recovery](/docs/blocks/auth/password-recovery) Blocks, or [compare Starters](/starters) for a full application foundation.
