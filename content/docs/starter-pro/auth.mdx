...
<Callout title="Core idea">
  Auth is not UI. Auth is lifecycle, security, state, redirects, provider rules,
  and trust.
</Callout>
...
<Callout type="info" title="Why this matters">
  Production auth is not only about signing in. It is about storing enough state
  to make account lifecycle, recovery, and provider logic safe.
</Callout>
...
<Callout title="Upgrade when auth becomes real">
  Starter Free helps you design the auth surface. Starter Pro gives you the auth
  system behind it.
</Callout>
...
<Callout type="info" title="Validation principle">
  Do not customize deeply before validating the default flows. Confirm the
  foundation first, then adapt the product.
</Callout>
...
<Accordion title="Can I remove a provider?">
  Yes. Providers should be treated as configurable entry points. Keep safe
  account fallback rules if you allow users to disconnect providers.
</Accordion>

<Accordion title="Should billing depend on auth?">
  Yes. In a SaaS, billing state must be connected to a user, customer, or
  organization identity. Auth is the foundation for that ownership.
</Accordion>
...
<DocsLinks
  ...
  items={[
    ...
    },
    {
      ...
      description: "Review the full Starter Pro foundation.",
      icon: Package,
    },
  ]}
/>
