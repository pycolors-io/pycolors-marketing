...
<Callout title="Release principle">
  A PWA is not production-ready because a manifest exists. It is
  production-ready when installability, offline fallback, mobile UX, and
  security boundaries have been validated.
</Callout>
...
<Callout type="info" title="Screenshot rule">
  Use real dashboard or workspace screenshots. Avoid using only login screens
  because they do not communicate the full SaaS value.
</Callout>
...
<Accordion title="Should I cache authenticated routes?">
  Not by default. Authenticated SaaS routes can contain sensitive or stale data.
  Start with fallback-only offline behavior.
</Accordion>
  ...
  items={[
    {
      ...
      description: "Understand the PWA-ready product layer in Starter Pro.",
      icon: Smartphone,
    },
    {
      title: "PWA setup",
