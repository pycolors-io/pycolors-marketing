...
<Callout title="Why this matters">
  Without tokens, every component slowly starts making its own visual decisions.
  That creates drift, makes refactors risky, and weakens the premium feeling of
...
## Pair surfaces with their foreground tokens

Use `primary` with `primary-foreground`, and `destructive` with
`destructive-foreground`. Their foregrounds use dark text in dark mode because
those semantic surfaces become lighter; hard-coded white text does not retain
readable contrast. The light-mode `muted-foreground` also supports text on the
muted surface. Custom palettes, opacity and layered backgrounds still require
contrast checks in their actual context.
...
export function ProductCard() {
  return (
    <article className="rounded-lg border bg-card p-6 shadow-sm">
      <h2 className="text-lg font-semibold tracking-tight">Product surface</h2>
...
<Callout title="Why the surface is intentionally small">
  A smaller token system is easier to understand, document, and maintain.
  PyColors adds tokens when they clarify the system, not when they create extra
  vocabulary.
</Callout>
...
<Accordion title="Should every design value become a token?">
  No. Tokens should represent repeated system decisions. Tokenizing every value
  too early creates complexity without improving consistency.
</Accordion>
...
<DocsLinks
  ...
  items={[
    ...
    },
    {
      ...
      description:
        "Generate semantic CSS overrides to place after the public token stylesheet import.",
      icon: ArrowRight,
    },
    {
      title: "Typography",
