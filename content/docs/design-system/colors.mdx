...
<Callout title="Why this matters">
  Color consistency is one of the fastest signals of product quality. A semantic
  color system keeps dashboards, marketing pages, documentation, forms, and
  reusable components visually aligned without forcing every component to
  reinvent its own palette.
</Callout>
...
<Steps>
  <Step>#### Understand the semantic role model behind PyColors colors.</Step>
  <Step>
  ...
  <Step>
    #### Use decision rules to keep UI colors, state colors, and chart colors
    separate.
  </Step>
</Steps>
...
<Callout type="info" title="Core principle">
  Components should consume color roles. The token layer should decide how those
  roles look.
</Callout>

<Callout type="info" title="Generate a semantic starting point">
  Need a concrete starting set of light and dark semantic values? Use the
  <Link href="/tools/theme-builder">PyColors Theme Builder</Link> to generate
  client-side CSS overrides, then review the result in your complete product
  context.
</Callout>
...
<Callout title="Why this matters">
  Separating UI roles from chart roles prevents data visualization colors from
  leaking into product states. A chart palette should not become the source of
  truth for buttons, alerts, or destructive actions.
</Callout>
...
export function ProductNotice() {
  return (
    <div className="rounded-2xl border border-border bg-card p-5 text-card-foreground">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        The visual intent comes from the design system, not from local
        overrides.
      </p>
    </div>
  );
...
<Callout type="warn" title="Common pitfall">
  Do not solve repeated color needs with local overrides. If the same visual
  intent appears in multiple places, define or adjust the semantic role instead.
</Callout>
...
<Callout title="Inline CTA">
  Ready to move from visual consistency to a production-shaped SaaS foundation?
  Explore <Link href="/docs/starter-pro">Starter Pro</Link> for auth, billing,
  backend, and delivery patterns built on top of PyColors UI.
</Callout>
...
<Accordion title="Reusing chart colors for product states">
  Chart colors are for data visualization. Product states such as success,
  warning, and destructive actions need dedicated semantic roles so feedback
  stays consistent.
</Accordion>

<Accordion title="Treating accent as decoration">
  `accent` should communicate intentional emphasis. If every section invents a
  different accent behavior, the product starts to feel inconsistent.
</Accordion>

<Accordion title="Fixing one screen instead of the token layer">
  Local fixes solve the current screen but create long-term maintenance cost. If
  the need repeats, update the semantic role centrally.
</Accordion>
...
<DocsLinks
  ...
  items={[
    ...
    },
    {
      ...
      description:
        "Generate contrast-aware semantic overrides and review them in a production-shaped preview.",
      icon: Palette,
    },
    {
      title: "UI components",
