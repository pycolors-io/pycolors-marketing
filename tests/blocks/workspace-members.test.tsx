...
describe("WorkspaceMembersPanel", () => {
  it("composes a named section, coherent headings and labelled member details", () => {
    ...
    expect(
      within(rows[0]!).getByRole("heading", { level: 3 }),
    ...
    expect(
      within(rows[1]!).getByRole("heading", { level: 3 }),
  ...
  it("uses stable member keys and follows consumer reordering without changing data", () => {
    ...
    const { rerender } = render(
      <WorkspaceMembersPanel {...props} members={[alex, members[1]!]} />,
    );
    const button = screen.getByRole("button", { name: "View Alex" });
    button.focus();
    rerender(
      <WorkspaceMembersPanel {...props} members={[members[1]!, alex]} />,
    );
    expect(
      screen
        .getAllByRole("heading", { level: 3 })
        .map((heading) => heading.textContent),
    ).toEqual(["Jordan Lee", "Alex Morgan"]);
      ...
      <form onSubmit={onSubmit}>
        <WorkspaceMembersPanel
          ...
          members={[
            {
              ...members[0]!,
              actions: <button type="button">View member</button>,
            },
          ]}
        />
      </form>,
  ...
  it.each([undefined, null, false, [], ""])(
    "omits absent section and member action groups (%s)",
    (actions) => {
      const { container } = render(
        <WorkspaceMembersPanel
          {...props}
          actions={actions}
          members={[{ ...members[0]!, actions }]}
        />,
      );
      expect(
        container.querySelector('[data-slot="workspace-members-actions"]'),
      ).toBeNull();
  ...
  it("renders an empty state without a list or unsolicited announcements and preserves section actions", () => {
    const { container, rerender } = render(
      <WorkspaceMembersPanel {...props} members={[]} />,
    );
  ...
  it("keeps IDs unique across panels, including panels containing the same member IDs", () => {
    const { container } = render(
      <>
        <WorkspaceMembersPanel {...props} description="First team" />
        <WorkspaceMembersPanel
          {...props}
          id="other-team"
    ...
    for (const name of [props.heading, "Other members"]) {
      ...
      expect(region).toHaveAttribute(
        "aria-labelledby",
        within(region).getByRole("heading", { level: 2 }).id,
      );
    }
  });
  ...
  it("has no axe violations in representative populated and empty compositions", async () => {
    const { container, rerender } = render(
      <WorkspaceMembersPanel
        {...props}
        actions={<a href="/guide">Team guide</a>}
      />,
    );
...
describe("WorkspaceMembersExample", () => {
  it("exercises the real canonical Block with fictional local actions and stable focus", () => {
    ...
    const button = screen.getByRole("button", {
      name: "View example details for Alex Morgan",
    });
    button.focus();
    fireEvent.click(button);
    expect(button).toHaveFocus();
    expect(screen.getByRole("status")).toHaveTextContent(
      "Selected example member: Alex Morgan. No data was changed.",
    );
    const disabled = screen.getByRole("button", {
      name: /View example details for Casey Park/,
    });
    expect(disabled).toBeDisabled();
    fireEvent.click(disabled);
    expect(screen.getByRole("status")).toHaveTextContent(
      "Selected example member: Alex Morgan.",
    );
  ...
  it("keeps repeated demos independent and their IDs unique", () => {
    const { container } = render(
      <>
        <section aria-label="First demo">
          <WorkspaceMembersExample />
        </section>
        <section aria-label="Second demo">
          <WorkspaceMembersExample />
        </section>
      </>,
    );
    ...
    expect(first.getByRole("status")).toHaveTextContent(
      "Selected example member: Alex Morgan.",
    );
    expect(second.getByRole("status")).toHaveTextContent(
      "No example member selected.",
    );
    const ids = Array.from(
      container.querySelectorAll("[id]"),
